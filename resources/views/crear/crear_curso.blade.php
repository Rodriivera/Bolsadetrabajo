...
<div class="perfil-contenedor separador" style="margin-top: 100px;">
    ...
    <form id="form-crear-curso" action="{{ route('bolsadetrabajo.cursos.store') }}" method="POST">
        ...
        <div class="form-group">
            ...
            <input type="text" name="institucion" class="form-control @error('institucion') input-error @enderror" value="{{ old('institucion') }}" placeholder="Ubicacion de la institución">
            @error('institucion')
            <small class="error-mensaje">{{ $message }}</small>
            @enderror
        </div>
